...
export const AssignableUsers = (params: queryVariables.AdminFields) => `
 	assignableUsers(${params.query ? `, query: "${params.query}"` : ""} ${params.after ? `, after: "${params.after}"` : ""} ${params.before ? `, before: "${params.before}"` : ""} first: ${params.first}, ${params.last ? ", last:${params.last}" : ""}) {
		edges {
			cursor
			node {
				${params.fields}
